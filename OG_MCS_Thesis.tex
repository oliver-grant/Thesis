...
\textbf{Split} is a top-down algorithm for solving the problem when there are $n$ levels in the memory hierarchy. Thite claims a running time of $O(2^n)$. We note that this running time result is dubious as Thite's explanation simply states that a root is chosen by examining each of the $2^{n-1}$ ways of partitioning memory locations between the left and right subtrees and recursing on each side. It is unclear how the potential roots are compared, and we cannot think of a way to select the correct root in this fashion without an exponential running time. A proof of correctness of algorithm was unfortunately omitted.
...
For any key $B_i$, if
\begin{align*}
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq location(B_i) \text{ then} \\
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq \frac{2}{p_i}-1.
\end{align*}
Let $par(B_{i-1}, B_i)$ represent the parent of the node for gap $(B_{i-1}, B_i)$ in $T$. If $n \geq 1$, then for any gap $(B_{i-1}, B_i)$, if 
\begin{align*}
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq location(par(B_{i-1}, B_i)) \text{ then} \\
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \text{ and} \\
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor. \text{ for at least $m$ of them (and the two extremal gaps)}.
\end{align*}

\end{lem}

\begin{proof}
Knowing a key's depth, its location in $T$ can be bounded as follows:
...
&\text{such that } \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \right) 
\end{align*}
\begin{align*}
...
&\text{such that } \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \right) \\
& \text{for all gaps, and}
\end{align*}
\begin{align*}
C(B_{i-1},B_i) \leq \sum_{k'=1}^{k-1} &\left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ 
\left(lg(\frac{1}{q_i}) + 1 - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k \\
&\text{such that } \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor \right) \\
...
C(B_{i-1},B_i) \leq \sum_{k'=1}^{k-1} &\left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ \\
 &\left(lg(\frac{1}{q_i}) + 2 - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k \\
&\text{such that } \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq location(par(B_{i-1},B_i)) \right)
\end{align*}
...
\begin{align*}
k=&min_{l \in \{1, ..., h\}} \mid m'_l \geq location(B_i) \text{ then} \\
&k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{1}{p_i}-1 \text{ if } p_i \in R \\
&k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \text{ otherwise.}
\end{align*}
Let $par(B_{i-1}, B_i)$ represent the parent of the node for gap $(B_{i-1}, B_i)$ in $T$. If $n \geq 1$, then for any gap $(B_{i-1}, B_i)$, if 
\begin{align*}
k=&min_{l \in \{1, ..., h\}} \mid m'_l \geq location(par(B_{i-1}, B_i)) \text{ then} \\
&k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{1}{q_i}-\frac{1}{2} \rfloor \text{ if } q_i \in R \\
&k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \text{ else if } p_i \neq q_{max} \\
&k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor. \text{ otherwise}.
\end{align*}
...
&\text{such that } k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{1}{p_i}-1 \text{ if } p_i \in R \\\\
 \sum_{k'=1}^{k-1} &\left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ \\
 &\left(\lg(\frac{1}{p_i}) + 1 - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k\\
&\text{such that } k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \text{ otherwise.}  
\end{cases}
\end{align*}
\begin{align*}
C((B_{i-1},B_i)) \leq
...
&\text{such that } k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{1}{q_i}-\frac{1}{2} \rfloor \text{ if } q_i \in R  \\\\
  \sum_{k'=1}^{k-1} &\left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ \\
 &\left(\lg(\frac{1}{q_i}) + 2 - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k\\
&\text{such that } k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \text{ else if } q_i \neq q_{max}  \\\\
 \sum_{k'=1}^{k-1} &\left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ \\
 &\left(\lg(\frac{1}{q_i})+1 - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k\\
&\text{such that } k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor \text{ otherwise.}
\end{cases}
\end{align*}
\end{lem}
...
The model treats leaves and internal nodes the same, charging $1$ for each node that must be examined when searching through the tree from the root for a specific node.
...
For completeness we formally prove its optimality under the \textit{Expected Path Length Model}. Note that email correspondence with Mordecai Golin significantly helped in the simplification of this proof \cite{golin2016}:

\begin{lem}\label{MSSolvesOpt}
The tree $T$ created by \textit{GREEDY-MS} for the multiset of probabilities $M$ solves the multiset binary tree problem under the \textit{Expected Path Length Model} optimally.
\end{lem}
...
First, we must prove a lemma for optimal trees under the \textit{Standard Model}.
...
\end{lem}
\begin{proof}
The proof follows by induction on the height of $T$. When $T$ has height $1$, it trivally has a single leaf node with $p_1=1$, and thus,  $\sum_{i \in L_T}p_i =1 \leq \frac{1+1}{2\cdot1}=1$ as required.
Suppose for all $T$ with heights strictly less than $l$, the claim holds. Consider any $T$ with height equal to $l$. One of three cases arise:

\begin{enumerate}
\item If our tree has two leaf children, then the probability of the root must be at least $\frac{1}{3}$ (since it must be bigger than its children) and $n=3$. Thus, 
\begin{align*}
\sum_{i \in L_T}p_i \leq \frac{2}{3} \leq \frac{2}{3}=\frac{n+1}{2n}
\end{align*}

\item If our tree has one leaf child, and one non-leaf child, let $p$ be the total probability of the sum of the weights of all nodes in the non-leaf subtree. We know our leaf (call it $l$) has probability at most $\frac{1-p}{2}$ since the probability of the root is greater. Thus, using our induction hypothesis we get that:
\begin{align*}
\sum_{i \in L_T}p_i &\leq \frac{n-2+1}{2\cdot(n-2)}\cdot p + \frac{1-p}{2} \\
&\leq \frac{n-1}{2\cdot(n-2)}\cdot p + \frac{1-p}{2} \\
&\leq \frac{1}{2} \cdot \left( \frac{n-1}{n-2}\cdot p + 1-p \right)\\
&\leq \frac{1}{2} \cdot \left( \frac{n\cdot p - p  + n - 2 -n\cdot p + 2p}{n-2}\right)\\
&\leq \frac{1}{2} \cdot \left( \frac{n-2+p}{n-2}\right)\\
&\leq \frac{1}{2} \cdot \left( \frac{n-1}{n-2}\right) \text{ since } p \leq 1\\
&\leq \frac{n+1}{2n} \text{ as required}
\end{align*}



\item If our tree has two non-leaf children, then let $p$ be the total probability of the left child subtree, and $q$ the probability of the right child subtree. Let $n_p$ be the number of nodes in the left subtree, and $n_q$ the number of nodes in the right subtree. By our note earlier, our root node must have probability at least as large as the larger of its two children, so it must be at least $\frac{1}{3}$. Thus, by our induction hypothesis, the maximum weight of the leaves is:
\begin{align*}
\sum_{i \in L_T}p_i &\leq \frac{n_p+1}{2n_p}\cdot p + \frac{n_q+1}{2n_q}\cdot q \\
&\leq \frac{n_p+1}{2n_p}\cdot \frac{1}{3} + \frac{n_q+1}{2n_q}\cdot \frac{1}{3} \text{ since } p \text{ and } q \text{ are smaller than the root} \\
&\leq \frac{1}{6} + \frac{1}{6\cdot n_p} + \frac{1}{6} + \frac{1}{6\cdot n_q} \\
&\leq \frac{1}{6} + \frac{1}{12} + \frac{1}{6} + \frac{1}{12} \text{ since } n_q \text{ and } n_p \text{ are both at least } 2 \\
&\leq \frac{1}{2} \leq \frac{n+1}{2n} \forall n > 0 
\end{align*}

\end{enumerate}

Combining the above three cases, we get that $\sum_{i \in L_T}p_i \leq \frac{n+1}{2n}$ by induction.
