...
First, we consider a new cost model for this problem, the \textit{\textbf{E}xpected Path Length Model}, which has cost $P_{T_E}$ defined as follows:
\begin{equation}
P_{T_E} = \sum_{i=1}^{n} p_i(b_i+1)
\end{equation}
...
\begin{thm}
The \textit{GREEDY-MS} Algorithm is within $1$ of optimal under the \textit{Standard Model}\ for cost.
\end{thm}
...
\begin{align*}
P_{T_S} = P_{T_E} - \sum_{i \in L_T}p_i
\end{align*}
Since the sum of the probabilities of the leaves of the tree is at most $1$ and at least $0$ we get:
\begin{align*}
P_{T_E} \leq P_{T_S} \leq P_{T_E} + 1
\end{align*}

Suppose for contraction of our theorem that there exists $T'$ has a cost strictly less than the cost of $T-1$ under the \textit{Standard Model}:
\begin{align*}
P_{T'_S} < P_{T_S} - 1
\end{align*}

Consider $T'$ under the \textit{Expected Path Length Model}:
\begin{align*}
P_{T'_E} = P_{T'_S} + \sum_{i \in L_T'}p_i \leq P_{T'_S} + 1 < P_{T_S} \leq P_{T_E}
\end{align*}
