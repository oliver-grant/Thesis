...
\Else
	...
	\While{$total < m_{i+1}$ and $\neg (empty?$ $r)$}
		\State $old\_loc = loc$
		\State $(loc, new\_r\_q) =$ AMWBT\textendash Range-Pack $(r_queue[0,...,0], i-1, loc, m_i)$
...
\For{$ j = 0$ to $l-1$}
	\While{$loc < m_{j+1}$ and $\neg(empty? r_q)$}
		\If{$loc + m_1 < m_{j+1}$}
			...
			\State $(loc, new\_r\_q) =$ AMWBT\textendash Range-Pack $(r_queue[0,...,0], i-1, loc, m_i)$		
		\Else
			\State $(loc, new\_r\_q) =$ AMWBT\textendash Range-Pack $(r_queue[0,...,0], 0, loc, m_i)$
		\EndIf
		\State $r\_q.pop()$
		\For{each node $r$ in $new\_r\_q$}
			\State $r\_q.push(r)$
