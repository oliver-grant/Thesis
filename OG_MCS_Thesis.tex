...
\begin{algorithm}
\caption{ApproxMWPaging with BT Packing}
\label{CHalgorithm}
\begin{algorithmic}[1]
\Procedure{AMWBT\textendash Pack}{$r\_queue[], i, loc, amt$}
\If{$i = 0$}
	\State Create largest BFS tree $T$ possible from $r\_q[0]$ with at most $min(loc, amt)$ nodes
	\State Push $T$ in BFS order into memory starting from memory location $loc$
	\State $loc += size(T)$
	\State $r\_q.pop()$
	\For{each leaf $l$ in $T$ in BFS order}
		\State Push all children of $l$ onto $r$
	\EndFor
	\State return $(r\_q[], loc)$
...
\Else
	...
	\While{$total < m_{i+1}$ and $\neg (empty?$ $r)$}
		\State $old\_loc = loc$
		\State $(loc, new\_r\_q) =$ AMWBT\textendash Pack $(r_queue[0,...,0], i-1, loc, m_i)$
