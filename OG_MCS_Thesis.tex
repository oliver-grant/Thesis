...
\begin{align*}
Q = \{q_i : q_i \text{ among smallest }m \text{ gaps excluding the extremal leaves} \} \cup \{q_0, q_n\}
\end{align*}
...
\begin{align*}
C \leq &\sum_{i=1}^{n} p_i\cdot  \sum_{j=1}^{k_{1,i}} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_j + \\ &\sum_{i=1}^{n+1} q_i\cdot \sum_{j=1}^{k_{2,i}} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_j \\
\text{such that } &\left( k_{1,i}=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \right) \text{ and} \\
&\left( k_{2,i}=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \right)
\end{align*}
