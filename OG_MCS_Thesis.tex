...
\begin{align*}
(\lg(\frac{1}{p_i}) - \lfloor \lg(m'_{k-1}+1) \rfloor)\cdot c_k.
\end{align*}
...
\section{The GREEDY-MS Algorithm is Within $1$ of Optimal}\label{OPT-MSBST}
...
\begin{thm}
The GREEDY-MS Algorithm is within $1$ of optimal under the \textit{Standard Model}\ for cost.
\end{thm}

\begin{proof}
Let $T$ be our tree created by GREEDY-MS. By \ref{MSSolvesOpt} we know that it solves the problem under the \textit{Expected Path Length Model} optimally. Let $L_T$ be the set of leaves of $T$. Consider the cost of $T$ under the \textit{Standard Model}:
\begin{align*}
