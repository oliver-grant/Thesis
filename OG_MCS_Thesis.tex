...
\begin{proof}
Since we already know that \textit{OPT-MSBST} provides an optimal solution, all that remains is to show that the tree $T$ created is unique up to permutations described. Suppose for contradiction there exists an optimal tree $Z$ for probability multiset $p$ with $f_z(p_i) \neq f(p_i)$ and there does not exists a permutation of assignments of nodes with the same probabilities as $p_i$ which gives $f_z(p_i) = f(p_i)$. This leaves us with two cases; $f_z(p_i) < f(p_i)$ or $f_z(p_i) > f(p_i)$. If $f_z(p_i) < f(p_i)$
\end{proof}
