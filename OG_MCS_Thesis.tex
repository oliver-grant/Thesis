...
\noindent Control is transferred to \uppercase{label1}, \uppercase{label2} or \uppercase{label3} if $expr < 0$, $expr=0$, or $expr > 0$ respectively using a single comparison command\footnote{Modern computers typically do not support such commands using a single three-way branch and will instead do at least two comparisons.}. The difference between a two-way branch model is significant and can be seen through a simple example of searching among 3 keys.
...
Assume the probability of search for any of $a,b$, or $c$ is $\frac{1}{3}$. Under the three-way branch model, this can be done using exactly one comparison by asking if the key we are search for is less than $b$, equal to $b$, or strictly greater than $b$ and returning the correct key appropriately. Under a standard two-way branch model, this would require an extra comparison operation $\frac{2}{3}$ of the time as we can only distinguish one of the the three cases from the other two using a single two-way $<, >, \leq$ or $\geq$. We get an expected cost of $1$ comparison in the three-way branch model and $\frac{5}{3}$ comparisons in the two-way model. More specifically, each comparison can reveal $3$ bits of information in the three-way model, while only $2$ bits per comparison can be revealed in the two-way model. 

\section{Why Study Binary Search Trees}

Binary search trees are ubiquitous throughout computer science with numerous applications. The basic binary search tree has extended in a number of ways. AVL trees (named after creators AdelsonVelskii and Landis) were the first form of self-balancing binary search trees introduced \cite{adelsonvelskii1963algorithm}. This type of tree was invented by the pair in 1963 and maintains a height of $O(\lg(n))$ (where $n$ is the number of nodes in the tree) during insertions and deletions (both of which take $O(\lg(n))$ time). Improved self-balancing binary search trees followed in the form of red-black trees by Bayer in 1972 \cite{bayer1972symmetric} and splay trees by Sleator and Tarjan in 1985 \cite{sleator1985self}. Tango trees were invented in 2007 by Demaine et al. and provided the first $O(\lg \lg n)$-competitive binary tree \cite{demaine2007dynamic}. Here, $O(\lg \lg n)$-competitive means that the tango tree does at most $O(\lg \lg n)$ times more work (pointer movements and rotations) than an optimal offline tree. B trees are among the most commonly used binary tree variant and were invented in 1970 by Bayer and McCeight \cite{bayer1970organization}. \\
...
Optimum alphabetic trees is an important related problem relevant later in this thesis. Given a set of $n$ keys (words $B_1, ..., B_n$) with various probabilities, we wish to build a binary search tree where every internal node has two children, leaves have no children, and the $n$ keys described are the leaves. We wish to build the tree with the minimum expected search cost. The expected search cost is
\begin{align*}
\sum_{i=1}^{i=n} p_i \cdot d_T(B_i)
\end{align*}
where $p_i$ is the probability of searching for word $B_i$ and $d_T(B_i)$ is the depth of the leaf representing the word $B_i$ in the tree $T$. The alphabetic ordering of the leaves must be maintained. This is the same as the binary search tree problem with all internal node weights zero.

In 1952, Huffman famously developed the Huffman tree, which solved the same problem without a lexicographic ordering constraint on leaves \cite{huffman1952method}. Gilbert and Moore examined the problem with the added alphabetic constraint and developed a $O(n^3)$ algorithm which solved the problem optimally \cite{gilbert1959variable}. Hu and Tucker gave a $O(n^2)$ time and space algorithm in 1971 \cite{hu1971optimal} which was improved by Knuth to take only $O(n \lg n)$ time and $O(n)$ space in 1973 \cite{knuth1973sorting}. The original proof of Hu and Tucker was extremely complicated, but was later simplified by Hu \cite{hu1973new} and Hu et al. \cite{hu1979binary}. Garsia and Wachs gave an independent $O(n \lg n)$ time, $O(n)$ space algorithm in 1977 \cite{garsia1977new}. This new algorithm by Garsia and Wachs was shown to be equivalent to the Hu and Tucker algorithm in 1982 by Hu \cite{Hu1982Book} and also went through a proof simplification \cite{kingston1988new} by Kingston in 1988.
...
\noindent where $x_i$'s represent successful search keys, pairs $(x_{i-1},x_i)$ represent unsuccessful search "keys" and $d_T(x_i)$  or $d_T(x_{i-1},x_i)$ represent the depth of a specific successful or unsuccessful searches respectively for a key $x_i$ or a key $(x_{i-1},x_i)$ in the tree $T$.
...
In this Chapter I show that the Modified Minimum Entropy Heuristic of G{\"u}ttler, Mehlhorn and Schneider \cite{guttler1980binary} is within an additive factor of entropy: at worst $H+4$.
...
\end{align*} to describe the entropy of any probability distribution $(x_1, x_2, ...., x_n)$. For sub tree $t$, we let 
\begin{align*}
p_t=\sum_{i : B_i \in t} p_i + \sum_{i : (B_i, B_{i+1}) \in t} q_i
\end{align*}
 be its total probability (the sum of the probability of all nodes within the subtree). $P_{L}(B_i)$ and $P_{R}(B_i)$ are probabilities of searching for a word lexicographically before or after (respectively) word $B_i$. $P_{L}(B_i, B_{i+1})$ and $P_{R}(B_i, B_{i+1})$ are probabilities of searching for a word lexicographically before (or equal to) $B_i$ and after (or equal to) $B_{i+1}$ respectively. For a subtree  $t$ $P_{L_t}(B_i)$ and $P_{R_t}(B_i)$ describe the normalized probabilities of searching for a word to the left or right of the root respectively. Formally,
\begin{align*}
...
\end{equation} be the local entropy of a sub tree $t$ rooted at word $B_i$.  

\section{The Modified Entropy Rule}\label{The Modified Entropy Rule}

As described in \cite{guttler1980binary}, the modified entropy heuristic (ME) greedily chooses the word $B_i$ as the root in one of the following three ways:

\begin{enumerate}[label=\alph*)]
\item If there exists word $B_i$ such that $\frac{p_i}{p_t} > \max(P_{L_t}(p_i), P_{R_t}(p_i))$ we always select $B_i$ as the root.
...
\item Otherwise, $B_i$ is selected such that $H(P_{L_t}(B_i), \frac{p_i}{p_t}, P_{R_t}(B_i))$ is maximized.
...
2) There exists word gap $(B_i, B_{i+1})$ such that $\frac{q_i}{p_t} > max(P_{L_t}(B_i, B_{i+1}), P_{R_t}(B_i, B_{i+1}))$\\
3)  $\max(P_{L_t}(B_r), P_{R_t}(B_r)) < \frac{4}{5}$\\
\end{lem}
...
Thus,
\begin{align*}
max(P_{L_t}(p_i), P_{R_t}(p_i)) \geq 1/3
\end{align*}
 and by our assumption 
\begin{align*}
max(P_{L_t}(p_i), P_{R_t}(p_i)) < \frac{1}{2}.
\end{align*}
...
$\implies F = - (\frac{1}{2}-\frac{1}{2} x)\cdot \lg(\frac{1}{2}-\frac{1}{2} x) - (\frac{1}{2} + \frac{1}{2} x)\cdot\lg(\frac{1}{2} + \frac{1}{2} x) - (1 - \frac{4}{5} (x)^2)$ \\
...
$\implies C \leq H + 2 + \sum\limits_{m = 0}^n \frac{4}{5} q_m \sum\limits_{x=0}^{\infty} \frac{1}{2} \cdot (\frac{4}{5}) ^ x$ \\
$\implies C \leq H + 2 + \sum\limits_{m = 0}^n \frac{4}{5}q_m (\frac{1}{2} \cdot \frac{1}{1-\frac{4}{5}})$ (geometric series) \\
$\implies C \leq H + 4$ \\
...
He provided three separate optimum solutions; \textbf{Parts}, \textbf{Trunks}, and \textbf{Split}. These algorithms have various use cases and running of times $O(\frac{2^{h-1}}{(h-1)!}\cdot n^{2h+1})$, $O(\frac{2^{n-m_h}\cdot (n-m_h+h)^{n-m_h}\cdot n^3}{(h-2)!})$ and $O(2^n)$ respectively. Here, $h$ is the minimum memory level such that all $n$ keys can fit on memories $\leq h$: \\
\begin{center}
$min(h \in \{1,..,l\} ): n \leq \sum_{i = 1}^{h}m_i$
\end{center}


 In the following sections, we will provide two approximate solutions to this problem that run in time $O(n\cdot\lg(m_1))$ and provide an upper bound on their expected search costs.  \\

Thite also considered the same problem under the related HMM$_2$ model. This model assumes there are simply two levels of memory of size $m_1$ and $m_2$ with costs of access $c_1$ and $c_2$ where $c_1 < c_2$. Thite provided an optimal solution to this problem (named \textbf{TwoLevel}) which ran in time $O(n^5)$. It ran in $o(n^5)$, if $m_1 \in o(n)$, and in $O(n^4)$ if $m_1 \in O(1)$. He also gave an $O(n\lg n)$ time approximate solution with an upper bounded expected search cost of $c_2(H+1)$. The solution we provide under the HMM model also provides an improvement over Thite's approximate algorithm in both running time and expected cost under the HMM$_2$ model.
...
2) Inside each page (node of the multiway tree $T'$), we create a balanced binary search tree (ignoring weights). We use a simple greedy approach where we sort the keys, then recursively select the middle key as the root.  We call each of these $T'_k$ for $k \in {1,...,\lceil n/m_1 \rceil}$. This takes $O(m_1\cdot\lg(m_1))$ time per page, of which there are at most $O(n/m_1)$ giving us $O(n\cdot\lg(m_1))$ time total. \\

3) In order to make this into a proper binary search tree, we must connect the $O(n/m_1)$ BST's we have made as described in Lemma~\ref{MWTBSTLem}. From $T'$, we create a BST $T$. This takes $O(n\cdot\lg(m_1))$ time. \\
...
$W \leq \sum_{i=1}^{l-1} (\lfloor \lg(m'_i+1) \rfloor - \lfloor \lg(m'_{i-1}+1) \rfloor)\cdot c_i+ (H(T) - \lfloor \lg(m'_{l-1}+1) \rfloor)\cdot c_l$
\end{lem}
...
\begin{center}
$(\lfloor \lg(m_j + 1) \rfloor - \lfloor \lg(m'_{j-1} + 1) \rfloor) \cdot c_j$.\\
\end{center}
...
\begin{center}
$(\lg(H(T) - \lfloor \lg(m'_{l-1}+1) \rfloor)\cdot c_l$.
\end{center}
...
$W \leq \lfloor \lg(m_1 + 1) \rfloor \cdot c_1 + \sum_{i=2}^{l-1} (\lfloor \lg(m'_i+1) \rfloor - \lfloor \lg(m'_{i-1}+1) \rfloor)\cdot c_i+ (H(T) - \lfloor \lg(m'_{l-1}+1) \rfloor)\cdot c_l$
$\implies W \leq \sum_{i=1}^{l-1} (\lfloor \lg(m'_i+1) \rfloor - \lfloor \lg(m'_{i-1}+1) \rfloor)\cdot c_i+ (H(T) - \lfloor \lg(m'_{l-1}+1) \rfloor)\cdot c_l$
...
\begin{center}
$W \leq H(T)\cdot c_l - \sum_{i=1}^{l-1} \lfloor \lg(m'_i+1) \rfloor \cdot(c_{i+1} - c_i) $
\end{center}
Since we have that $c_i > c_{i-1}$ for all $i$, $\sum_{i=1}^{l-1} \lfloor \lg(m'_i+1) \rfloor \cdot(c_{i+1} - c_i)$ is strictly positive. This instantly gives the result desired.
\end{proof}
...
\begin{center} $C(x_i) \leq (\lg(\frac{1}{p_i})+1)\cdot \frac{W}{H(T)} < (\lg(\frac{1}{p_i})+1)\cdot c_l$ 
\end{center}


\begin{center}
$C(x_{i-1},x_i) \leq (\lg(\frac{1}{q_i})+2)\cdot \frac{W}{H(T)} < (\lg(\frac{1}{q_i})+2)\cdot c_l$ for all leaves, and \\
$C(x_{i-1},x_i) \leq (\lg(\frac{1}{q_i})+1)\cdot \frac{W}{H(T)} < (\lg(\frac{1}{q_i})+1)\cdot c_l$ for at least $m$ of them (and the two extremal leaves).
...
$C(x_i) \leq (\lg(\frac{1}{p_i})+1)\cdot\frac{W}{H(T)}$   \\
$\implies C(x_i) < (\lg(\frac{1}{p_i})+1)\cdot\frac{H(T)\cdot c_l}{H(T)}$   \\
$\implies C(x_i) < (\lg(\frac{1}{p_i})+1)\cdot c_l$

For leaves we have that\\

$C(x_{i-1},x_i) \leq (\lg(\frac{1}{q_i})+2)\cdot \frac{W}{H(T)}$   \\
$\implies C(x_{i-1},x_i) \leq (\lg(\frac{1}{q_i})+2)\cdot \frac{H(T)\cdot c_l}{H(T)}$   \\
$\implies C(x_{i-1},x_i) < (\lg(\frac{1}{q_i})+2)\cdot c_l$ for all leaves, and \\
$\implies C(x_{i-1},x_i) < (\lg(\frac{1}{q_i})+1)\cdot c_l$ for at least $m$ of them (and the two extremal leaves).
...
$C \leq  (\frac{W}{H(T)} \cdot  (H + 1 + \sum_{i=0}^n q_i - q_0 - q_n - \sum_{i=0}^m q_{rank[i]})$ \\
and \\
$C < (H + 1 + \sum_{i=0}^n q_i - q_0 - q_n - \sum_{i=0}^m q_{rank[i]}) \cdot  c_l$ \\
...
$C \leq \sum_{i=1}^{n} p_i\cdot C(x_i) + \sum_{j=1}^{n+1} q_j\cdot C(x_{i-1},x_i)$ \\

$\implies C \leq \sum_{i=1}^{n} p_i\cdot (\lg(\frac{1}{p_i})+1)\cdot \frac{W}{H(T)} + (\sum_{i=0}^{n} q_i(\lg(\frac{1}{q_i})+2) - q_0 - q_n -  \sum_{i=0}^m q_{rank[i]} )\cdot \frac{W}{H(T)} $ \\
...
$\implies C < (H + 1 + \sum_{i=0}^n q_i - q_0 - q_n - \sum_{i=0}^m q_{rank[i]}) \cdot  c_l$ \\
...
$W \leq \sum_{k=1}^{l-1} (\lfloor \lg(m'_i+1) \rfloor - \lfloor \lg(m'_{i-1}+1) \rfloor)\cdot c_i+ (H(T) - \lfloor \lg(m'_{l-1}+1) \rfloor)\cdot c_l$ 
\end{lem}
...
Since we have the same result as Lemma~\ref{452}, this immediately implies Lemma~\ref{W<HT} is true as well. Assuming that $l \neq 1$, we have that $W<H(T)\cdot c_l$. As in the proof of the expected cost ApproxMWPaging, $\frac{W}{H(T)}$ represents the average cost per memory access when accessing the deepest (and most costly) element of our tree. Thus, $\frac{W}{H(T)}$ upper bounds the average cost per memory access when searching for any element of $T$.
\\

\begin{thm} \label{ApproxBSTThm}
$C \leq  (\frac{W}{H(T)}) \cdot  (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})$ and \\
$C <  (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})\cdot c_l$
\end{thm}
...
$C \leq \sum_{i=1}^{n}(p_i\cdot \frac{depth(p_i)+1}{H(T)}\cdot  W)+ \sum_{i=0}^{n}(q_i\cdot \frac{(depth(q_i))}{H(T)}\cdot  W)$ \\

$\implies C \leq \frac{W}{H(T)}(\sum_{i=1}^{n}(p_i\cdot (depth(p_i)+1))+ \sum_{i=0}^{n}(q_i\cdot (depth(q_i))))$ \\

$\implies C \leq \frac{W}{H(T)}(Average Path Length(T))$ \\

$\implies C \leq  (\frac{W}{H(T)}) \cdot  (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})$ \\
and by Lemma~\ref{W<HT} \\
$\implies C <  (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})\cdot c_l$
\end{proof}
...
Since both \\
\begin{center}
$ C_{ApproxMWPaging} < (H + 1 + \sum_{i=0}^n q_i - q_0 - q_n - \sum_{i=0}^m q_{rank[i]}) \cdot  c_2 < c_2(H+1+\sum_{i=0}^{n}q_i)$
\end{center}
 and
 \begin{center}
 $C_{ApproxBSTPaging} < (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})\cdot c_2 < c_2(H+1+\sum_{i=0}^{n}q_i)$
 \end{center}

and both ApproxMWPaging and ApproxBSTPaging run in time $o(n\lg(n))$, both methods provide a strictly better approximation and run faster than the \textit{Algorithm Approx-BST} of Thite.
...
$T_{P} \in \sum_{j=0}^{l-1})sum_{q \in S_j} O(q\_amt\cdot j)$ \\
$\implies T_{P} \in o(l) \cdot  \sum_{j=0}^{l-1})sum_{q \in S_j} O(q\_amt)$ \\
...
\begin{center}
$P_{T'}-P_T = (p_j - p_k)\cdot (f(p_k)-f(p_j))$
\end{center}
\end{lem}
...
$P_{T'}-P_T = \sum_{i=1}^{n} p_i\cdot f'(p_i) - \sum_{i=1}^{n} p_i\cdot f(p_i)$ \\
$= p_j\cdot f'(p_j)+p_k\cdot f'(p_k) - p_j\cdot f(p_j)+p_k\cdot f(p_k)$\\
$= p_j\cdot f(p_k)+p_k\cdot f(p_j) - p_j\cdot f(p_j)+p_k\cdot f(p_k)$\\
$= p_j\cdot (f(p_k)-f(p_j)) + p_k (f(p_j) + f(p_k))$\\
$= p_j\cdot (f(p_k)-f(p_j)) - p_k (f(p_k)-f(p_j))$\\
$= (p_j - p_k)\cdot (f(p_k)-f(p_j))$
\end{proof}
...
$ = \frac{1}{1-p_{min}}\cdot \sum_{p_i \in (p - \{p_{min}\})}p_i$\\
...
$P_T = \sum_{p_i \in (p - \{p_{min}\})} p_i\cdot f(p_i) + p_{min}\cdot f(p_{min})$ \\
$= \sum_{p_i \in (p - \{p_{min}\})} p'_i\cdot (1-p_{min})\cdot f'(p_i) + p_{min}\cdot f(p_{min})$ \\
$= (1-p_{min})\cdot \sum_{p'_i \in p'} p'_i\cdot f'(p_i) + p_{min}\cdot f(p_{min})$ \\
$= (1-p_{min})\cdot P_{T'} + p_{min}\cdot f(p_{min})$ \\
...
$P_Z = \sum_{p_i \in p)} p_i\cdot f_z(p_i)$ \\
$= \sum_{p_i \in (p - \{p_{min}\})} p_i\cdot f_z(p_i) + p_{min}\cdot f_z(p_{min})$ \\
as with $T'$, we can similarly say that: \\
$= \sum_{p_i \in p'} (1-p_{min})\cdot p'_i\cdot f_z(p_i) + p_{min}\cdot f_z(p_{min})$ \\
...
$P_Z < P_T$ \\
$\implies (1-p_{min})\cdot P_{Z'} + p_{min}\cdot f_Z(p_{min}) < (1-p_{min})\cdot P_{T'} + p_{min}\cdot f(p_{min})$ \\
$\implies P_{Z'} < P_{T'}$ \\
...
\begin{center}
    ...
    \textbf{ApproxMWPaging}  & HMM  & $\mathbf{O(n\cdot \lg(m_1))}$   & $\mathbf{C < (H + 1 + \sum_{i=0}^n q_i - q_0 - q_n - \sum_{i=0}^m q_{rank[i]}) \cdot  c_l}$    \\ \hline
    \textbf{ApproxBSTPaging}  & HMM & $\mathbf{O(n)}$    & $\mathbf{C < (H + 1 - q_0 - q_n + q_{max} - \sum_{i=0}^{m'} pq_{rank[i]})\cdot c_l}$    \\ \hline
    \textbf{ApproxMWPaging with BT}  & HMBTM  &  $\mathbf{O(n\cdot l)}$   & TODO \\ \hline
    \textbf{ApproxBSTPaging with BT}  & HMBTM  & TODO   & TODO    \\ \hline
    \textbf{OPT-MSBST}  & RAM  & $\mathbf{O(n\cdot \lg(n))}$    & ---    \\ \hline
    \end{tabular}
\end{center}
