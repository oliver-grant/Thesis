...
$C \leq \sum_{i=1}^{n}(p_i*\frac{depth(p_i)}{H(T)}* W)+ \sum_{i=0}^{n}(q_i*\frac{depth(q_i)}{H(T)}* W)$ \\

$\implies C \leq \frac{W}{H(T)}(\sum_{i=1}^{n}(p_i*depth(p_i))+ \sum_{i=0}^{n}(q_i*depth(q_i)))$ \\
...
BFS each balanced BST in $T'$

Use pages from $T'$

Create pages of pages of T' by doing BFS of  $T'$, pages are of size $m_2/m_1$, call this $T'_2$

Recursively do this until we have $T'_l$

An objects location is their BFS page order in $T'_l * m_l$ + ... + BFS page order in blah does this work?

MAYBE I HAVE TO DO IT FROM $m_l $ backwards? I think that may do it.
