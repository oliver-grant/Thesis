...
Search(curr, key, block_index_stack, offset_stack)
	mem <- correct child of (curr - offset_stack.top()) #empty if no child
	if mem < m_1 # if we are in the upper most block we are fine
		if mem = empty return curr		
		else return search(mem, key, block_index_stack, offset_stack)
		
	# this pops what block size we are using until we get a big enough one
	while (! block_stack.empty() AND (mem > m'[block_stack.top()]))
		offset_stack.pop()
		block_index_stack.pop()

	if block_stack.empty() 
		new_block_i = min i such that m_i' > mem # this can be sped up by remembering, if empty always bigger
		block_index_stack.push(new_block_i)
		offset_stack.push(curr)
		[mem, mem + m_{i-1}-1] -> [m'_{i-2}, m'_{i-1}-1]
		return search(mem, key, block_index_stack, offset_stack)
	else
		offset_stack.pop()
...
pack(r[], i, loc)
	if i = 0
		from root_q[0] pack up to m_1 keys into memory in BFS order
		loc += amount packed
		root_q.pop()
...
loc = 0
for j = 0 to l-1
	while (total < m_{j+1} and !r.empty())
		(loc,r_q) = pack(root_q[0], max i such that total + m_i < m_{j+1}, loc)
		for each r in r_q root_q.push(r)
