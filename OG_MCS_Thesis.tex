...
k=min_{l \in \{1, ..., h\}} \mid m'_l &\geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor. \text{ for at least $m$ of them (and the two extremal leaves)}.
\end{align*}
...
location(par(B_{i-1},B_i)) &\leq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \text{ for all leaves.}
\end{align*}
...
\begin{align*} 
C(B_i) &\leq  \sum_{j=1}^{k} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_k \mid \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \right)\\
C(B_{i-1},B_i) &\leq \sum_{j=1}^{k} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_k \mid \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \right) \\
& \text{   for all leaves, and} \\
C(B_{i-1},B_i) &\leq \sum_{j=1}^{k} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_k \mid \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{2}{q_i}-\frac{1}{2} \rfloor \right) \\
...
\begin{align*}
C(B_{i-1},B_i) &\leq \sum_{k'=1}^{k-1} \left(\lfloor \lg(m'_{k'}+1) \rfloor - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_{k'}+ \left(d_T(B_i) - \lfloor \lg(m'_{k'-1}+1) \rfloor \right)\cdot c_k \\
...
\begin{align*}
C \leq &\sum_{i=1}^{n} p_i\cdot  \sum_{j=1}^{k} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_k \\
&\mid \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \frac{2}{p_i}-1 \right) + \\
&\sum_{j=1}^{n+1} q_j\cdot \sum_{j=1}^{k} \left(\lfloor \lg(m'_j+1) \rfloor - \lfloor \lg(m'_{j-1}+1) \rfloor \right)\cdot c_k \\
&\mid \left( k=min_{l \in \{1, ..., h\}} \mid m'_l \geq \lfloor \frac{4}{q_i}-\frac{1}{2} \rfloor \right)
\end{align*}

TODO GOTTA FIX THAT SHIT TO SUBTR

\end{thm}

\begin{proof}
The total expected cost of search is simply the sum of the weighted cost of search for all keys multiplied by the probability of searching for each key. Plugging in \ref{AMWPExactLem} we get:
\begin{align*}
