...
\begin{equation}
C = \sum_{i=1}^{n} p_i \cdot (d_T(B_i)+1) + \sum_{i=0}^{n} q_i \cdot d_T \big( (B_i, B_{i+1}) \big)
\end{equation}
\end{center}
\end{frame}
...
\begin{align*}
H &= \sum_{t \in S_T} p_t E_t \scriptsize{\text{ (sub in } E_t \text{ bound)}} \\
&\geq \sum_{t \in S_T} p_t \cdot \left(1 - \frac{b_t}{p_t} \right) = C - \sum_{t \in S_T} b_t \\
...
$C =\sum_{i=1}^{n} p_i \cdot (d_T(B_i)+1) + \sum_{j=0}^{n} q_j \cdot d_T((B_{i-1},B_i))$
...
\only<2->{
\begin{align*}
C =\sum_{i=1}^{n} p_i \cdot C(B_i) + \sum_{i=0}^{n} q_i \cdot C\left( (B_{i-1}, B_i) \right)
\end{align*}}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Directly from work of Bose and DouDou\"{i}eb \cite{bose2009efficient}
\end{itemize}
...
\begin{lem}
For a BST $T$ with all parent probabilities greater than their children's:
\begin{align*}
...
\begin{tikzpicture}[level/.style={sibling distance = 6cm/####1,
  level distance = 1.5cm}] 
\node (Root) [arn_n, fill=red] {$0.40$ \\ THE}
    child{ node [arn_x, fill=red] {$0.35$ \\ $(-\infty, THE)$}
            }
...
\node (Root) [arn_n] {$0.30$ \\ THE}
    child{ node [arn_n,  fill=red] {$0.20$ \\  $BE$} edge from parent[draw=none]
            child{ node [arn_x,  fill=red] {$0.10$ \\ $(-\infty, BE)$}} 
            	child{ node [arn_x] {$0.06$ \\  $(BE, THE)$}}
            }
    child{ node [arn_n, fill=blue] {$0.20$ \\ $TO$} edge from parent[draw=none]
            child{ node [arn_x, fill=blue] {$0.10$ \\ $(THE,TO)$}}
            child{ node [arn_x] {$0.04$ \\ $(TO, \infty)$}}
		}};
		\only<2>{
\node (Root) [arn_n, fill=green] {$0.30$ \\ THE}
    child{ node [arn_n, fill=red] {$0.20$ \\  $BE$} 
            child{ node [arn_x, fill=red] {$0.10$ \\ $(-\infty, BE)$}} 
            	child{ node [arn_x, fill=green] {$0.06$ \\  $(BE, THE)$}}
            }
    child{ node [arn_n, fill=blue] {$0.20$ \\ $TO$}
            child{ node [arn_x, fill=blue] {$0.10$ \\ $(THE,TO)$}}
            child{ node [arn_x] {$0.04$ \\ $(TO, \infty)$}}
		}};
\end{tikzpicture}
