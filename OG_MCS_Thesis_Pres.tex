...
\item Ordering property over the keys allows for quick searches
...
\item Knuth proposed the optimum binary search tree problem in 1971 \cite{knuth1971optimum}
...
\node [arn_n] {0.2 CAT}
    child{ node [arn_n] {0.2 BAT} 
            child{ node [arn_x] {0.1 \\ $x < BAT$}} 
            	child{ node [arn_x] {0.1 \\  $BAT < x < CAT$}}
            }
    child{ node [arn_n] {0.1 DOG}
            child{ node [arn_x] {0.1 \\ $CAT < x < DOG$}}
            child{ node [arn_x] {0.2 \\ $x > DOG$}}
		}
...
{\tiny
\begin{align*}
P = &p_{BAT} \cdot (d_T(BAT)+1) + p_{CAT} \cdot (d_T(CAT)+1) +p_{DOG} \cdot (d_T(DOG)+1) + \\
 &q_{(-\infty, BAT)} \cdot d_T((-\infty, BAT)) + q_{(BAT, CAT)} \cdot d_T((BAT, CAT)) + q_{(CAT, DOG)} \cdot d_T((CAT, DOG)) +q_{(DOG, \infty)} \cdot d_T((DOG, \infty)) \\
 = &0.2 \cdot 2 + 0.2 \cdot 1 + 0.1 \cdot 2 + \\
 & 0.1 \cdot 2 + 0.1 \cdot 2 + 0.1 \cdot 2 + 0.2 \cdot 2
\end{align*}
}%
...
\item Improve bound on MME Heuristic of G{\"u}ttler, Mehlhorn and Schneider \cite{guttler1980binary}

\item Optimum BST under Memory Model of Aggarwal et al. \cite{aggarwal1987model}

\item Two $O(n)$ time approximation algorithms and bound their cost

\item Optimum BST problem variant where probabilities are unordered

\item Under \textit{Standard Model}, \textit{GREEDY-MS} within $\frac{n+1}{2n}$ of optimal
