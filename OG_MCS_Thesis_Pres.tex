...
C &\leq H + \sum_{t \in S_T} b_t \\
 &\leq H + 2 \cdot \sum_{i=1}^{n} p_i + 4 \cdot \sum_{j=0}^{n} q_j \\
 &\leq H + 4
\end{align*}
\end{frame}
...
For a given depth $d$, the probabilities of all nodes at depth $d+1$ are \textbf{less than or equal to} the probabilities of all nodes at depth $d$.}
\uncover<2->{
\begin{proof}
...
\item[\textit{Case 3)}]  $\max(P_{L_t}(B_r), P_{R_t}(B_r)) < \frac{4}{5}$. \textbf{We set $b_t = \frac{4}{5} \cdot \frac{q_i^2}{p_t}$ where $q_i$ is the middle gap.}
...
\begin{align*}
C &\leq H + \sum_{t \in S_T} b_t = H + 2 \sum\limits_{r = 1}^n p_r + 2 \sum\limits_{i = 0}^n q_i + \sum\limits_{m = 0}^n \sum\limits_{t \in S_i} \frac{4}{5}\frac{q_i^2}{p_t}\\
C &\leq H + 2 + \sum\limits_{m = 0}^n (\frac{4}{5} \cdot q_i) \sum\limits_{x=0}^{\infty} \frac{1}{2} \cdot (\frac{4}{5}) ^ x \\
C &\leq H + 2 + 2 \\
C &\leq H + 4
\end{align*}
\end{frame}
